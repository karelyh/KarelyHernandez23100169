...
exports.pemCertificates = pemCertificates;
exports.pemToDer = pemToDer;
exports.bareCertificate = bareCertificate;
exports.toPem = toPem;
exports.findAncestorNsForElement = findAncestorNsForElement;
...
const BOM_REGEX = /^(?:\uFEFF|\u00EF\u00BB\u00BF)/;
function normalizePemInput(text) {
    return text
        .replace(BOM_REGEX, "")
        .replace(/\r\n|\r/g, "\n")
        .split("\n")
...
function countOpenings(text) {
    return text.split("-----BEGIN").length - 1;
}
function pemMessages(pem) {
    const messages = [];
 ...
 */
function isX509Certificate(bytes) {
    try {
        new crypto_1.X509Certificate(bytes);
        return true;
    }
    catch {
        return false;
...
function assertX509Certificate(data) {
...
function isWellFormedMessage({ label, endLabel, headers, data }) {
...
function formatPemMessage(label, data) {
...
function bareCertificate(value) {
...
function findSubsetNSPrefixes(subset) {
...
function findAncestorNsForElement(node) {

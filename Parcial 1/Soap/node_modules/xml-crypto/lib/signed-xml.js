...
function findSignatureElements(node) {
...
function certificatesToPublish(publicCert) {
...
const warnOriginalXmlWithIds = (0, util_1.deprecate)(() => { }, "`getOriginalXmlWithIds()` is deprecated and will be removed in a future version. Use the `location` option of `computeSignature()` to place the signature, then `getSignedXml()`. For a detached signature, put an ID attribute the signer recognizes on each referenced element (`wsu:Id` for WS-Security), sign that document, and send it alongside `getSignatureXml()`.", "XML_CRYPTO_GET_ORIGINAL_XML_WITH_IDS");
const warnValidateElementAgainstReferences = (0, util_1.deprecate)(() => { }, "`validateElementAgainstReferences()` is deprecated and will be removed in a future version. After `checkSignature()` succeeds, use the XML that `getSignedReferences()` returns instead of nodes from the original document.", "XML_CRYPTO_VALIDATE_ELEMENT_AGAINST_REFERENCES");
class SignedXml {
    /**
    ...
    ensureTargetsHaveIds(referenceTargets) {
        for (const [ref, targets] of referenceTargets) {
            if (!ref.isEmptyUri) {
    ...
    calculateReferenceDigest(ref, node) {
        ref.ancestorNamespaces = utils.findAncestorNsForElement(node);
        const canonXml = this.canonicalize(ref.transforms, node, {
            inclusiveNamespacesPrefixList: ref.inclusiveNamespacesPrefixList,
            ancestorNamespaces: ref.ancestorNamespaces,
        }, { discardComments: true });
        return this.findHashAlgorithm(ref.digestAlgorithm).getHash(canonXml);
    }
    findSignatureContentTargets(ref, doc, signatureElem) {
        const nodes = xpath.selectWithResolver(ref.xpath ?? "", doc, this.namespaceResolver);
        isDomNode.assertIsArrayOfNodes(nodes);
        return nodes
    ...
    addAllReferences(doc, signatureElem, referenceTargets, prefix) {
        ...
        const signatureContentTargets = new Map();
    ...
    canonicalize(transforms, node, options, { discardComments }) {
        ...
        if (transforms.includes("http://www.w3.org/2000/09/xmldsig#enveloped-signature")) {
            ...
            while (signatureAncestor?.parentNode && signatureAncestor !== node) {
                signaturePath.push(Array.from(signatureAncestor.parentNode.childNodes).indexOf(signatureAncestor));
        ...
        for (const transformName of transforms) {
            ...
            transformedXml = this.findCanonicalizationAlgorithm(transformName).process(transformedXml, transformOptions);
        }
        if (isDomNode.isNodeLike(transformedXml)) {
            transformedXml = this.findCanonicalizationAlgorithm("http://www.w3.org/TR/2001/REC-xml-c14n-20010315").process(transformedXml, transformOptions);
        }
    ...
    findLoadedSignature(node) {
        ...
        const matches = findSignatureElements(doc).filter((signature) => utils.findChildren(signature, "SignatureValue")[0]?.textContent === signatureValue);
        if (matches.length > 1) {
            throw new Error("Cannot validate a document which contains multiple Signature elements with the same SignatureValue, in order to prevent signature wrapping attack.");
        }
        return matches[0] ?? null;
    }
    parseTransformInput(octets, transformName) {
        const parseErrors = [];
        ...
        if (parseErrors.length > 0 || doc.documentElement == null) {
            throw new Error(`Cannot apply transform ${transformName}: the output of the previous transform is not well-formed XML`);
        }
